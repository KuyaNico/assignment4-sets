...
TEST_CASE("SetType Tests") {
    ...
    SECTION("Difference Tests - 5pts") {
        ...
        for (int i = 75; i <= 99; i++) {
            ...
            // Stuff that should be in A - B
            if (!diffAB.Contains(i)) {
                INFO("expected diffAB set to contain " << i);
                FAIL();
            }

            // Stuff that should NOT be in B - A
            if (diffBA.Contains(i)) {
                INFO("diffBA should not contain " << i);
                FAIL();
            }
        }
        ...
        for (int i = 0; i <= 24; i++) {
            ...
            // Stuff that should be in B - A
            if (!diffBA.Contains(i)) {
                INFO("expected diffBA set to contain " << i);
                FAIL();
            }

            // Stuff that should NOT be in A - B
            if (diffAB.Contains(i)) {
                INFO("diffAB should not contain " << i);
                FAIL();
            }
        }
    }
    ...
    SECTION("Rehash Tests - 5pts") {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (testSet.LoadFactor() != load) {
                ...
                INFO("Your Load Factor: " << std::setprecision(12) << testSet.LoadFactor());
                INFO("........Expected: " << std::setprecision(12) << load);
                FAIL();
            }
        }
    }
